  ...
  resetKeywordsVisibility(isUnviel: boolean) {
    ...
    if(!this.isUnviel) {
      for (let i = 0; i < this.keyTexts.length; i++) {
        this.keyTexts[i].textFill.color = new vec4(textColor.x, textColor.y, textColor.z, 0); // Set initial color to transparent
        };
    }else {
      for (let i = 0; i < this.keyTexts.length; i++) {
        this.keyTexts[i].textFill.color = new vec4(textColor.x, textColor.y, textColor.z, 1); 
      }
    }
  }
    ...
    Base64.encodeTextureAsync(
      ...
      (base64String) => {
        ...
        if (firstUnfoundIndex === -1) {
          print("All keywords found. No need to request.");
         return;
        }
